...
# Function to compute the sigmoid kernel matrix
def compute_sigmoid_kernel(X, X2, alpha=0.05, beta=1):
    return np.tanh(alpha * np.dot(X, X2.T) + beta)

# Function to train KELM with regularization
def train_kelm(X_train, y_train, reg_coeff, alpha=0.05, beta=1):
    K_train = compute_sigmoid_kernel(X_train, X_train, alpha, beta)
...
def predict_kelm(X_train, X_test, weights, alpha=0.05, beta=1):
    K_test = compute_sigmoid_kernel(X_test, X_train, alpha, beta)
...
regularization_coeffs = [0.0000000001, 0.000000001, 0.0000001, 0.0001]

# One-hot encode target if needed
encoder = OneHotEncoder(sparse=False)

# Create a Pandas Excel writer using XlsxWriter as the engine.
excel_file_path = 'E:\\BISMILLAHIRAHMANIRAHIM SKRIPSI\\KELM_Evaluation_Results.xlsx'
with pd.ExcelWriter(excel_file_path, engine='xlsxwriter') as writer:
    ...
    for reg_coeff in regularization_coeffs:
        ...
        for fold_number in range(1, 11):
            ...
            alpha = 0.05  # Assuming default for now, could be tuned later
            beta = 1
            weights = train_kelm(X_train, y_train_encoded, reg_coeff, alpha, beta)

            # Predict the test set using the trained KELM model
            y_pred_encoded = predict_kelm(X_train, X_test, weights, alpha, beta)
        ...
        results_df.to_excel(writer, sheet_name=f'Reg_Coeff_{reg_coeff}', index=False)

        # Print results for each regularization coefficient
        print(results_df)
