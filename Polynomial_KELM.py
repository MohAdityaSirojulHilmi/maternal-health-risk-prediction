...
data_path = 'E:\\BISMILLAHIRAHMANIRAHIM SKRIPSI\\KesehatanMaternaly.csv'
...
# KFold dengan 10 folds, shuffle=True, random_state agar hasil tidak berubah-ubah
...
# Function to compute the polynomial kernel matrix
def compute_polynomial_kernel(X, X2, degree=3, coef0=1):
    # Calculate the polynomial kernel matrix
    return (np.dot(X, X2.T) + coef0) ** degree

# Function to train KELM with regularization
def train_kelm(X_train, y_train, reg_coeff, degree=3, coef0=1):
    K_train = compute_polynomial_kernel(X_train, X_train, degree, coef0)
...
def predict_kelm(X_train, X_test, weights, degree=3, coef0=1):
    K_test = compute_polynomial_kernel(X_test, X_train, degree, coef0)
...
excel_file_path = 'E:\\BISMILLAHIRAHMANIRAHIM SKRIPSI\\Polynomial_KELM_Evaluation_Results.xlsx'
with pd.ExcelWriter(excel_file_path, engine='xlsxwriter') as writer:
    ...
    for reg_coeff in regularization_coeffs:
        ...
        for fold_number in range(1, 11):
            ...
            coef0 = 1   # Coefficient for the polynomial kernel
            weights = train_kelm(X_train, y_train_encoded, reg_coeff, degree, coef0)

            # Predict the test set using the trained KELM model
            y_pred_encoded = predict_kelm(X_train, X_test, weights, degree, coef0)
